...
struct basic_table : table {
    ...
    constexpr explicit basic_table(
            const char* name, unsigned flags = 0, MDB_cmp_func value_cmp = nullptr) noexcept :
    ...
    */
    template <typename D>
    expect<value_stream<V, D>> static get_value_stream(
            K const& key, std::unique_ptr<MDB_cursor, D> cur) noexcept {
        MONERO_PRECOND(cur != nullptr);
