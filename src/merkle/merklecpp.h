...
namespace merkle
{
  ...
  protected:
    ...
    struct Node
    {
      ...
      static Node* make(Node* left, Node* right)
      {
        ...
        r->swapped = false;
        //Ensure left child's hash is less than the right child's hash
  ...
  public:
    ...
    size_t find_leaf_index(const merkle::HashT<HASH_SIZE>& target_hash) const
    {
        for (size_t i = 0; i < leaf_nodes.size(); i++)
        {
            if (leaf_nodes[i]->hash == target_hash)  // Directly compare HashT objects now
  ...
  protected:
    ...
    void insert_leaves(bool complete = false)
    {
      if (!uninserted_leaf_nodes.empty())
      {
        ...
        // Sort the leaves based on their hash values before inserting
        std::sort(uninserted_leaf_nodes.begin(), uninserted_leaf_nodes.end(),
