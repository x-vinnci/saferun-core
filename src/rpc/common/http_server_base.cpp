...
std::optional<std::string> check_authorization(
        std::string_view auth_header, std::string_view realm, Callback check_login) {
    ...
    if (colon == std::string_view::npos)
        return fail;
    if (check_login(
                std::string_view{login}.substr(0, colon),
                std::string_view{login}.substr(colon + 1)))
        return std::nullopt;
    return fail;
}

bool http_server_base::check_auth(HttpRequest& req, HttpResponse& res) {
    if (auto www_auth = check_authorization(
...
void http_server_base::error_response(
        HttpResponse& res, http_response_code code, std::optional<std::string_view> body) const {
...
std::string http_server_base::get_remote_address(HttpResponse& res) {
    ...
    if (addr.size() == 4) {  // IPv4, packed into bytes
        ...
        // IPv6, packed into bytes.  Interpret as a series of 8 big-endian shorts and convert to
        // hex, joined with :.  But we also want to drop leading insignificant 0's (i.e. '34f'
        // instead of '034f'), and we want to collapse the longest sequence of 0's that we come
        // across (so that, for example, localhost becomes `::1` instead of `0:0:0:0:0:0:0:1`).
...
void http_server_base::handle_cors(
        HttpRequest& req, std::vector<std::pair<std::string, std::string>>& extra_headers) {
    if (m_cors_any)
