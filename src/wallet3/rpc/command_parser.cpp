...
void parse_request(TRANSFER& req, rpc_input in) {
  ...
  if (auto* json_in = std::get_if<json>(&in)) {
    if (auto dests = json_in->find("destinations");
        dests != json_in->end() && !dests->empty() && dests->is_array() && dests->front().is_object()) {

      for (auto& d : *dests)
        r.destinations.emplace_back(d["address"].get<std::string>(), d["amount"].get<uint64_t>());
    }
    else
  ...
  get_values(in,
      "do_not_relay", r.do_not_relay,
      "get_tx_hex", r.get_tx_hex,
      "get_tx_key", r.get_tx_key,
      "get_tx_metadata", r.get_tx_metadata,
