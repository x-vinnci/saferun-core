...
#error "Invalid SKEIN_UNROLL_256"               /* sanity check on unroll count */
    ...
    do {
        ...
        Skein_Show_R_Ptr(
                BLK_BITS, &ctx->h, SKEIN_RND_KEY_INITIAL, Xptr); /* show starting state values */
...
static int Skein_256_Update(Skein_256_Ctxt_t* ctx, const u08b_t* msg, size_t msgByteCnt) {
    ...
    Skein_Assert(
            ctx->h.bCnt <= SKEIN_256_BLOCK_BYTES, SKEIN_FAIL); /* catch uninitialized context */
...
static int Skein_256_Final(Skein_256_Ctxt_t* ctx, u08b_t* hashVal) {
    ...
    for (i = 0; i * SKEIN_256_BLOCK_BYTES < byteCnt; i++) {
        ...
        Skein_Put64_LSB_First(
                hashVal + i * SKEIN_256_BLOCK_BYTES, ctx->X, n); /* "output" the ctr mode bytes */
...
static int Skein_512_Update(Skein_512_Ctxt_t* ctx, const u08b_t* msg, size_t msgByteCnt) {
    ...
    Skein_Assert(
            ctx->h.bCnt <= SKEIN_512_BLOCK_BYTES, SKEIN_FAIL); /* catch uninitialized context */
...
static int Skein_512_Final(Skein_512_Ctxt_t* ctx, u08b_t* hashVal) {
    ...
    for (i = 0; i * SKEIN_512_BLOCK_BYTES < byteCnt; i++) {
        ...
        Skein_Put64_LSB_First(
                hashVal + i * SKEIN_512_BLOCK_BYTES, ctx->X, n); /* "output" the ctr mode bytes */
...
static int Skein1024_Update(Skein1024_Ctxt_t* ctx, const u08b_t* msg, size_t msgByteCnt) {
    ...
    Skein_Assert(
            ctx->h.bCnt <= SKEIN1024_BLOCK_BYTES, SKEIN_FAIL); /* catch uninitialized context */
...
static int Skein1024_Final(Skein1024_Ctxt_t* ctx, u08b_t* hashVal) {
    ...
    for (i = 0; i * SKEIN1024_BLOCK_BYTES < byteCnt; i++) {
        ...
        Skein_Put64_LSB_First(
                hashVal + i * SKEIN1024_BLOCK_BYTES, ctx->X, n); /* "output" the ctr mode bytes */
...
static SkeinHashReturn Update(hashState* state, const BitSequence* data, DataLength databitlen) {
    ...
    if ((databitlen & 7) == 0) /* partial bytes? */
    {
        ...
        switch ((state->statebits >> 8) & 3) {
            case 2:
                Skein_512_Update(
                        &state->u.ctx_512, data, bCnt - 1); /* process all but the final byte    */
                Skein_512_Update(&state->u.ctx_512, &b, 1); /* process the (masked) partial byte */
                break;
            case 1:
                Skein_256_Update(
                        &state->u.ctx_256, data, bCnt - 1); /* process all but the final byte    */
                Skein_256_Update(&state->u.ctx_256, &b, 1); /* process the (masked) partial byte */
                break;
            case 0:
                Skein1024_Update(
                        &state->u.ctx1024, data, bCnt - 1); /* process all but the final byte    */
